...
let serversmap = new Map(); // Lista de todos os jogos que estão funcionando no momento
    ...
    try {
        ...
        request('https://www.palabrasaleatorias.com/palavras-aleatorias.php/', async(err, res) => {
            ...
            while (word.length !== confirmation.length && points < 6) {
                ...
                if (!guess.size) {
                    await msg.reply('Aah que pena, o tempo acabou!');
                    break;
                }
            ...
            msg.delete(msg.channel.id);
            if (word.length === confirmation.length || guessed) return msg.reply(`você venceu, a palavra era ${word}!`);
            return msg.reply(`que pena, a palavra era ${word}...`);
    });
    } catch (err) {
        msg.delete(msg.channel.id);
        return msg.reply(`erro: \`${err.msg}\`. Tente novamente mais tarde!`);
    }
}
